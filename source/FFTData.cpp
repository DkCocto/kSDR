...
FFTData::FFTData(int startSpectreLen) { init(startSpectreLen); }
...
void FFTData::init(int startSpectreLen) {
  ...
  shadowWaterfallDataN =
      new OUTPUT{new float[startSpectreLen], startSpectreLen};
  memset(shadowWaterfallDataN->data, 0,
         sizeof(float) * shadowWaterfallDataN->len);
...
/// <summary>
/// ¬озвращает копию данных дл¤ спектра или водопада. ¬одопад или спектр
/// выбираетс¤ через задани¤ параметра waterfall в функцию. Ќе забывать
/// уничтожать данные после использовани¤!
/// </summary>
...
FFTData::OUTPUT *FFTData::getDataCopy(bool waterfall) {
  ...
  if (!waterfall) {
    memcpy(dataCopy->data, spectreDataN->data + sizeDiv2,
           sizeof(float) * sizeDiv2);
    memcpy(dataCopy->data + sizeDiv2, spectreDataN->data,
           sizeof(float) * sizeDiv2);
    ...
    memcpy(dataCopy->data, waterfallDataN->data + sizeDiv2,
           sizeof(float) * sizeDiv2);
    memcpy(dataCopy->data + sizeDiv2, waterfallDataN->data,
           sizeof(float) * sizeDiv2);
...
/// <summary>
/// ¬озвращает кусок из данных начина¤ с определенной позиции и определенной
/// длины. Ќе забывать уничтожать данные после использовани¤!
/// </summary>
...
/// <summary>
/// ¬озвращает кусок из данных начина¤ с определенной позиции и определенной
/// длины из предоставленных данных. Ќе забывать уничтожать данные после
/// использовани¤!
/// </summary>
...
/// <summary>
/// ѕолное удаление объекта OUTPUT из пам¤ти.
/// </summary>
/// <param name="data"></param>
...
/*std::vector<float> FFTData::getData(int startPos, int len, bool waterfall) {
        ...
        std::copy(data.begin() + startPos, data.begin() + startPos + len,
dataCopy.begin());
...
/*std::vector<float> FFTData::getData(bool waterfall) {
        ...
        int size = (waterfall == true) ? waterfallData.size() :
spectreData.size(); int sizeDiv2 = size / 2;

        std::vector<float> dataCopy(size);
        //memcpy(buffer, data + (spectreSize / 2), sizeof(data) * (spectreSize /
2));
        //memcpy(buffer + (spectreSize / 2), data, sizeof(data) * (spectreSize /
2)); if (!waterfall) { std::copy(spectreData.begin() + sizeDiv2,
spectreData.end(), dataCopy.begin()); std::copy(spectreData.begin(),
spectreData.end() - sizeDiv2, dataCopy.begin() + sizeDiv2);

                shadowSpectreData = dataCopy;
        } else {
                std::copy(waterfallData.begin() + sizeDiv2, waterfallData.end(),
dataCopy.begin()); std::copy(waterfallData.begin(), waterfallData.end() -
sizeDiv2, dataCopy.begin() + sizeDiv2);
